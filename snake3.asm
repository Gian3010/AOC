.data
    # --- VARIÁVEIS DE ESTADO (Reordenadas) ---
    var_pontos:      .word   0
    ...
    # --- DEFINIÇÃO DE CORES (Nomes alterados) ---
    COR_FUNDO:       .word   0x00cefad0      # Verde Claro
    COR_PAREDE:      .word   0x00008631      # Verde Escuro
    COR_COBRA:       .word   0x000000ff      # Azul
    COR_BLOCO:       .word   0x00505050      # Obstaculo
    COR_IMORTAL:     .word   0x00ffffff      # Branco (Cabeça imune)
    
    # --- ITENS COLECIONÁVEIS ---
    ITEM_PONTO:      .word   0x00ff0000      # Vermelho
    ITEM_GELO:       .word   0x00FF7F00      # Laranja (Slow)
    ITEM_ESTRELA:    .word   0x00ff00ff      # Magenta (Invencível)

    # --- VETORES DE DIREÇÃO (Mantendo lógica de bits altos) ---
    # Norte=1, Sul=2, Oeste=3, Leste=4 (nos bits superiores)
    ...
    # --- MENSAGENS ---
    txt_perdeu:      .asciiz "Fim de Jogo! Pontos: "
...
renderizar_cabeca:
    ...
    lw   $t9, var_imune
    blez $t9, cor_normal
    lw   $t0, COR_IMORTAL       # Branco se imune
    j    pintar_final
cor_normal:
    lw   $t0, COR_COBRA
pintar_final:
